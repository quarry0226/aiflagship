...
class TeamUpService:
    ...
    def get_my_info(self):
        ...
        if my_info_response.status_code == 200:
            return my_info_response.json()
        else:
            logger.error("내 정보를 받아오는 데 실패했습니다.")
            sys.exit()

    def get_my_room_info(self,room_index):
        response = self.client.get(
            edge_host + '/v3/room/{}'.format(room_index),  # 1 은 confirm
            timeout=self.config['lp_wait_timeout']
        )
        if response.status_code == 200:
    ...
    def get_chat_user_info(self,user_index,team_index):
        response = self.client.get(
            auth_host + '/v1/user/{}/team/{}'.format(user_index,team_index),  # 1 은 confirm
            timeout=self.config['lp_wait_timeout']
        )
        if response.status_code == 200:
            return response.json()
